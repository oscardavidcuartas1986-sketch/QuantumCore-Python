...
def dibujar():
    ...
    for f in range(PISOS, 0, -1):
        ...
        shaft_lines.append(
            f"  {BOLD}{WHITE}Piso {f}{RESET}  │ {cabina} │  {dot}"
        )
        shaft_lines.append(f"        │      │")
...
def ciclo_ascensor():
    ...
    while True:
        ...
        if estado == "q0":
            ...
            dibujar()
            if piso_alcanzado():
                agregar_log(f"OK Atendiendo piso {piso_actual}")
                ...
                if estado == "q0":
                    agregar_log("Sin mas solicitudes — en reposo")
                dibujar()
...
def control_usuario():
    ...
    while True:
        ...
        # Acepta varios pisos: "1 3 4" o "1,3,4" o "1, 3, 4"
        ...
        for token in tokens:
            try:
                ...
                if 1 <= piso <= PISOS:
                    ...
                    agregar_log(f"Error: piso invalido: {piso} (usa 1-{PISOS})")
            except ValueError:
                agregar_log(f"Error: '{token}' no es valido")
