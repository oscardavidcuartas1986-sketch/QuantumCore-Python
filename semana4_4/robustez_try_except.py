...
def leer_y_almacenar_datos_robusto(nombre_archivo="transacciones_corruptas.txt"):
    ...
    with archivo:
        ...
        for numero_linea, linea in enumerate(archivo, start=1):
            ...
            partes = [dato.strip() for dato in linea.split(",")]
            ...
            try:
                ...
                total_errores += 1
                print(
                    f"[ERROR - ValueError] Línea {numero_linea} "
                    f"('{linea}'): {str(error).rstrip('.')}. "
                    f"Registro descartado, continuando con la siguiente línea..."
                )
                continue
                ...
                total_errores += 1
                print(
                    f"[ERROR - TypeError] Línea {numero_linea} "
                    f"('{linea}'): {error}. "
                    f"Registro descartado, continuando con la siguiente línea..."
                )
                continue
    ...
    print("\n----- RESUMEN DE CARGA -----")
    print(f"Líneas procesadas : {total_lineas_procesadas}")
    print(f"Cargadas con éxito: {len(lista_transacciones)}")
...
datos_robustos = leer_y_almacenar_datos_robusto()
mostrar_transacciones(datos_robustos)
